...
class App {
  ...
  getSquareBounds(lat, lon, sideMeters) {
    const halfSide = Math.max(sideMeters, 20) / 2;
  ...
  updateSiteMap(r) {
    ...
    const landAreaM2 = Math.max(r.solar.landAreaM2 || 0, 0);
    const squareSideMeters = Math.sqrt(landAreaM2);
    ...
    if (!earthScenario) {
      this.showSiteMapMessage(
        ...
        'Satellite view is Earth-only. The footprint numbers above still reflect the modeled installation size.'
      );
      return;
    }
    ...
    if (!hasCoords) {
      this.showSiteMapMessage(
        ...
        'The highlighted square uses the current Earth coordinates as the center of the modeled footprint.'
      );
      return;
    }
    ...
    this.showSiteMap(
      'Satellite view for Earth locations. The highlighted square is a scale proxy for the modeled installation footprint.'
    );
